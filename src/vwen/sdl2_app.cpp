...
// Window parameters
char title[] = "SDL2 APP"; // window's title
...
void app_sdlError(const char *mes)
{
	fprintf(stderr, "%s: %s\n", mes, SDL_GetError());
	exit(1);
}
...
void app_Init()
{
	...
	if(SDL_Init(SDL_INIT_EVERYTHING) < 0)
		app_sdlError("Could not initialize SDL");
	...
	if(window == NULL)
		app_sdlError("Could not create window");
	...
	if(glContext == NULL)
		app_sdlError("Could not create the OpenGL context");
...
void app_OpenGLSet() // set up OpenGL
{
...
{
	app_Init(); // Init SDL2 Glad
	
	app_PreLoop(); // To do before loop (display graphics card info)
	
	app_OpenGLSet(); // set up OpenGL
